...
module.exports.run = async (client, message, args) => {
  ...
  if (!amountStarts || amountStarts < 1 || amountStarts > 5)
    return message.reply(`${language.cmd_review_no_stars}`);

  const messageReview =
    args.splice(1, args.length).join(" ") ||
    `**${language.cmd_review_no_msg}**`;

  const reviewChannel = message.member.guild.channels.cache.get(
