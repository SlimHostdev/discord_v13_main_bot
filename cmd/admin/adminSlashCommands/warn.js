...
module.exports = {
  ...
  async execute(client, interaction) {
    ...
    const warns = JSON.parse(
      fs.readFileSync("./src/data/warnings.json", "UTF8")
    );
    ...
    const Embed = new MessageEmbed()
      ...
      .setTimestamp()
      .setDescription(
        `**${language.cmd_warn_warn}** <@${warnUserId}> (${warnUserId})
        **${language.cmd_warn_reason}** ${reason}`
      )
      .addField(
        `${language.cmd_warn_number_warn} `,
        warns[warnUserId].warns.toString()
      );
    ...
    if (warns[warnUserId].warns == 3) {
      var mes = new discord.MessageEmbed()
        ...
        .setImage(process.env.BANNER)
        .setDescription(
          `${language.cmd_warn_watch_out} ` + userOption.user.username
        )
        .setColor(process.env.WARNCOLLOR)
        .setFooter(
          interaction.member.displayName,
      ...
      var banEmbed = new discord.MessageEmbed()
        ...
        .setTimestamp()
        .setDescription(
          `**${language.cmd_ban_banned_disc}** <@${warnUserId}> (${warnUserId})
      ...
      const banchannel = interaction.member.guild.channels.cache.get(
