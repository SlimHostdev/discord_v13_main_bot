...
module.exports.run = async (client, message, args) => {
  ...
  if (warnUser.roles.cache.has(`${process.env.ADMINROLL}`))
    return message.reply(`${language.cmd_warn_cant_warn_admin}`);
  ...
  var warnEmbed = new discord.MessageEmbed()
    ...
    .setTimestamp()
    .setDescription(
      `**${language.cmd_warn_warn}** <@${warnUser.id}> (${warnUser.id})
        **${language.cmd_warn_by}** ${message.author}
        **${language.cmd_warn_reason}** ${reason}`
    )
    .addField(
      `${language.cmd_warn_number_warn} `,
      warns[warnUser.id].warns.toString()
    );
  ...
  if (warns[warnUser.id].warns == 3) {
    var mes = new discord.MessageEmbed()
      ...
      .setImage(process.env.BANNER)
      .setDescription(
        `${language.cmd_warn_watch_out} ` + warnUser.user.username
      )
      .setColor(process.env.WARNCOLLOR)
      .setFooter(message.member.displayName, message.author.displayAvatarURL)
    ...
    var banEmbed = new discord.MessageEmbed()
      ...
      .setTimestamp()
      .setDescription(
        `**${language.cmd_ban_banned_disc}** <@${warnUser.id}> (${warnUser.id})
    ...
    const banchannel = message.member.guild.channels.cache.get(
