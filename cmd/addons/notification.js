...
module.exports.run = async (client, message, args) => {
  if (!message.member.roles.cache.has(`${process.env.ADMINROLL}`))
    return message.reply(`${notification.no_admin}`).then((msg) => {
      message.delete();
  ...
  if (!msg_notification)
    return message.reply(`**${notification.no_msg}**`).then((msg) => {
      message.delete();
      setTimeout(() => msg.delete(), 10000);
    });

  const notificationChannel = message.member.guild.channels.cache.get(
    notification.channel
  );

  if (!notificationChannel)
    return message.reply(`${notification.no_channel}`).then((msg) => {
      message.delete();
      setTimeout(() => msg.delete(), 10000);
    });
