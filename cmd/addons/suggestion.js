...
module.exports.run = async (client, message, args) => {
  ...
  if (!msg_suggestion)
    return message.reply(`**${suggestion.no_msg}**`).then((msg) => {
      message.delete();
      setTimeout(() => msg.delete(), 10000);
    });

  const suggestionChannel = message.member.guild.channels.cache.get(
    suggestion.channel
  );

  if (!suggestionChannel)
    return message.reply(`${suggestion.no_channel}`).then((msg) => {
      message.delete();
      setTimeout(() => msg.delete(), 10000);
    });
