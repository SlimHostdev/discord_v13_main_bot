...
module.exports.run = async (client, message, args) => {
  //Log chat
  const adminlog = message.member.guild.channels.cache.get(
    process.env.ADMINLOGS
  );
  ...
  if (!message.guild.me.permissions.has("KICK_MEMBERS"))
    return message.reply(`${language.cmd_kick_no_purm}`);
  ...
  var kickUser = message.guild.members.cache.get(
  ...
  var embedPrompt = new discord.MessageEmbed()
    ...
    .setDescription(
      `${language.cmd_kick_sure_disc} ${kickUser} ${language.cmd_kick_sure_disc2}?`
    )
    .setTimestamp();
  ...
  var embedKick = new discord.MessageEmbed()
    ...
    .setImage(process.env.BANNER)
    .setDescription(
      `**${language.cmd_kick_kicken_disc}** ${kickUser} (${kickUser.id})
  ...
  message.channel
    .send({ embeds: [embedPrompt], components: [row] })
    .then(async (msg) => {
      ...
      collector.on("collect", (interactionButton) => {
        ...
        switch (id) {
          case "Yes":
            ...
            kickUser.kick(reason).catch((err) => {
              ...
              return message.channel
                .send(`${language.cmd_kick_err} ${kickUser}`)
                .then((msg) => {
                  message.delete();
          ...
          case "No":
            ...
            message.channel
              .send(`${language.cmd_kick_dont_kick} ${kickUser} .`)
              .then((msg) => {
                message.delete();
                setTimeout(() => msg.delete(), 5000);
              });
