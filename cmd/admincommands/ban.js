...
module.exports.run = async (client, message, args) => {
  ...
  if (!message.guild.me.permissions.has("BAN_MEMBERS"))
    return message.reply(`${language.cmd_ban_no_purm}`);
  ...
  var banUser = message.guild.members.cache.get(
  ...
  var embedPrompt = new discord.MessageEmbed()
    ...
    .setDescription(
      `${language.cmd_ban_sure_disc} ${banUser} ${language.cmd_ban_sure_disc2}?`
    )
    .setTimestamp();
  ...
  var embedBan = new discord.MessageEmbed()
    ...
    .setImage(process.env.BANNER)
    .setDescription(
      `**${language.cmd_ban_banned_disc}** ${banUser} (${banUser.id})
      ...
      collector.on("collect", (interactionButton) => {
        ...
        switch (id) {
          ...
          case "No":
            ...
            message.channel
              .send(`${language.cmd_ban_dont_ban} ${banUser}.`)
              .then((msg) => {
                message.delete();
