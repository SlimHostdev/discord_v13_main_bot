...
module.exports.run = async (client, message, args) => {
  ...
  if (!reason)
    return message.channel.send(`${language.cmd_ticket_no_msg}`).then((msg) => {
      message.delete();
  ...
  message.guild.channels
    .create(userName.toLowerCase() + "-" + userDiscriminator, { type: "text" })
    .then((createChan) => {
      createChan
        .setParent(categoryID)
        .then((settedParent) => {
          // Perms zodat iedereen niets kan lezen.
          settedParent.permissionOverwrites.edit(
          ...
          settedParent.permissionOverwrites.edit(
            message.guild.roles.cache.find(
              (x) => x.id === `${process.env.TICKETSTAFF}`
            ),
            {
              VIEW_CHANNEL: true,
              CREATE_INSTANT_INVITE: false,
          ...
          let embedParent = new discord.MessageEmbed()
            ...
            .addFields(
              {
                name: `${language.cmd_ticket_create_at}`,
                value: today,
                inline: false,
              },
              {
                name: `${language.cmd_ticket_msg}`,
                value: reason,
                inline: false,
              }
            );
